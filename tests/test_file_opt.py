...
from coe_number import file_opt
...
class TestFileOpt(unittest.TestCase):
    ...
    @mock.patch("os.remove")
    def test_filename_is_lower_but_file_not_exist(self, os_remove, os_path_isfile):
        filename = mock.MagicMock()
        filename.isupper.return_value = False
        file_opt.rm_when_lower(filename)
    ...
    @mock.patch("os.path.isfile", return_value=True)
    @mock.patch("os.remove")
    def test_filename_mixed_case(self, os_remove, os_path_isfile):
        filename = mock.MagicMock()
        filename.isupper.return_value = False # Mixed case is not upper
        file_opt.rm_when_lower(filename)
    ...
    @mock.patch("os.path.isfile", return_value=True)
    @mock.patch("os.remove")
    def test_filename_with_numbers(self, os_remove, os_path_isfile):
        file_opt.rm_when_lower("123.txt")
        os_remove.assert_called()
